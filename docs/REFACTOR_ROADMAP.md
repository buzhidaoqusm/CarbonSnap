...
- [ ] **0.1 提交现有改动**：未提交的 A6/A7 工作拆成 2–3 个 commit（shadow 模块 + 测试 / 接入 conversation service / 报告脚本 + 文档）。
- [ ] **0.2 依赖管理换成 uv**：`backend/pyproject.toml` + `uv.lock`，替代 `requirements.txt`。Python 固定 3.12（本机系统 Python 是 3.14，部分依赖可能没有对应 wheel）。删除损坏的 `.venv`（指向已不存在的 `D:\anaconda3`）并重建。
- [ ] **0.3 容器化**：
  ...
  - 前端暂时保持本地 `vite`。
- [ ] **0.4 代码质量**：ruff（lint + format）、mypy（先只检查 `app/services/ai`，逐步扩大范围）、pre-commit。
- [ ] **0.5 CI**：GitHub Actions `ci.yml`，依次跑 ruff → mypy → pytest → vitest。
- [ ] **0.6 压测基线**（后面做对比全靠它）：
...
- [ ] **0.7 修掉已知失败的测试**：`test_ai_decision_engine.py::TestForumRetrieval::test_recycling_intents_enable_forum_retrieval`。
- [ ] **0.8 顺手修安全问题**（旧计划 D3、D4）：
